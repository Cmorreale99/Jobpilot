...
- **Pipeline audit (hardening H8 — the V4 readiness gate)**: `python -m app.tools.audit_pipeline` walks the whole persisted spine offline (zero LLM, zero network) and asserts every gate invariant (`domain/pipeline_audit.py` pure checks + `services/pipeline_audit.py`): **capture** (every active source row has an active `source_document_version` with raw text), **element_coverage** (every active version's tree covers its raw text exactly, every element disposed), **ownership_labeled** (no active assigned row without an `assignment_method`), **active_orphans** (every active row's chunk text recomputes from the active version's raw payload — structured rows via raw-coordinate spans, flat-era rows via normalized-text spans, spanless commits via containment), **reviewed_on_stale** (no APPROVED claim cites superseded evidence — rejections stay in the H6 queue but don't hold the gate), **provenance_walk** (approved story component → claim → claim_evidence → evidence → element closes; result quotes locate; attestation-backed components resolve), **version_consistency** (active rows/versions stamped with current `NORMALIZATION_VERSION`/`STRUCTURER_VERSION`). Scorecard recorded in `validation_runs` (kind `pipeline_audit`) — the gate's sign-off artifact; exit 1 on any failure, failures name their rows (capped at 20 per check). Negative-control tests (`tests/test_pipeline_audit.py`) prove each corruption fails its specific check. **Gate PASSED on the live u1 corpus 2026-07-11** (see `docs/research/PIPELINE_HARDENING_PLAN.md` §7). Enumeration surfaces added for the audit: `ClaimRepository.list_all_evidence`, `SourceCaptureStore.list_documents`.
...
| `STRUCTURED_ASSIGNMENT` | `true` | Hardening H5, the Cooper fix: chunks are cut from source elements (never across an element boundary; span refs are raw-text coordinates, `evidence.element_id/sequence_index/section_path` link the tree, migration `0019`) and ownership is decided once per top-level section subtree (`SectionAssigner` — heading beats body vocabulary, ties refused; LLM variant behind `ROSTER_LLM_DETECTION`), inherited by the section's chunks as `assignment_method='section'`. READMEs stay repo-owned by construction; structureless sources keep the per-chunk assigner; human pins (single chunk or `POST /roster/sections/{element_id}/assign` subtree) always hold. False restores the flat-text per-chunk path — the one-release rollback lever. |
...
| `PROBLEM_SPACE_LLM_DETECTION` | `false` | When false, problem-space grouping uses the lexical heuristic (`_related_problems` transitive union). True selects the LLM grouper (DEEP tier — partitions an entity's distinct problem statements semantically, by index only, sanitized so it can merge but never author/omit/duplicate; ≤1 small call per multi-problem entity; needs `LLM_ENABLED`; else it warns and stays heuristic). LLM partitions are **recorded** (`problem_space_groupings`, migration `0015`; `PersistedGroupingDetector` wraps the LLM path via the factory's `grouping_store`+`user_id`): synthesis, eval, and re-runs over an unchanged problem set replay one recording — no partition drift (live-verified failure mode 2026-07-11), no repeat LLM spend. Fingerprints are problem-SET based and namespaced by `GROUPING_PROMPT_VERSION` (bump on any prompt change); first write wins — re-grouping requires deleting the row. The heuristic path is never wrapped. |
...
- [ ] M23 — Second-Brain / Obsidian ingestion substrate: frontmatter + wikilink parsing, vault-zone policy; a personal knowledge vault becomes a policy-scoped evidence source alongside Drive/GitHub/uploads. **Prerequisite spec: `docs/research/claude_jobpilot_exhaustive_ingestion.md`** — exhaustive PDF/DOCX source capture (two-pass: verbatim `SourceElement` records with reconciliation and zero silent drops, THEN interpretation), the durable fix for the live chunk-boundary evidence losses (Cooper.ai results assigned to the wrong entity; "Paper recommender system: no evidence" while source text existed).
