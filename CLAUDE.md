...
| `PROBLEM_SPACE_LLM_DETECTION` | `false` | When false, problem-space grouping uses the lexical heuristic (`_related_problems` transitive union). True selects the LLM grouper (DEEP tier — partitions an entity's distinct problem statements semantically, by index only, sanitized so it can merge but never author/omit/duplicate; ≤1 small call per multi-problem entity; needs `LLM_ENABLED`; else it warns and stays heuristic). Synthesis and `run_story_eval` must run with the same detector (both factory-created) so the contamination invariant judges the partition that built the stories. |
