...
- [ ] M18 — V3 Phase 2: story domain layer (`domain/project_story.py`), live-shaped corpus fixture, `project_stories` + normalizer versioning (migration `0013`), story repos with never-replace-approved + invalidation (built; awaiting review)
- [ ] M19 — V3 Phase 3: heuristic story synthesis (`services/story_synthesis.py`: quarantine + `synthesis_hash` skip), review API (`api/stories.py`) with the runtime resume-ready **409 gate**, story-scoped answers (`story:{id}:{component}` attestations), roster merge/discard → `invalidate_story` cascade, `MISSING_ACTION` question, dashboard story cards (`web/components/stories.tsx`) (built; awaiting review)
- [ ] M22 — V3 Phase 5b (LLM synthesis, credit-gated): flag-gated `LlmStorySynthesizer` (`llm/story_synthesis.py`, DEEP tier — composes the Problem Space + groups Actions, **Results stay verbatim**; grounds composed text against cited evidence and drops ungrounded numbers/vague problems; loud `StorySynthesisError` on LLM failure), behind `StorySynthesizer` protocol + `HeuristicStorySynthesizer` default + `create_story_synthesizer` factory (flag `STORY_LLM_SYNTHESIS`); `run_story_synthesis` takes the synthesizer + a `failed` report bucket; `eval_stories` (`domain/evaluation.py::summarize_story_eval` + `services/story_eval.py`, `validation_runs` kind `story_eval`: resume_ready_rate, invented_metric/orphan/duplicate counts all 0-or-flagged). **LIVE run done (2026-07-07)**: real Opus over the u1 corpus → 15 stories, 8 resume-ready, invented_metric=0/orphan=0, duplicate_story_count=1 (the known OneWorld/portfolio dup). (built + live-verified; awaiting review)
- [ ] M21 — V3 Phase 5a (downstream, offline half): Story→`MasterCv` adapter (`story_master_cv_from_snapshot`, `source_ref=story:<id>:<component_id>`, cited `evidence_text` baked into the snapshot), pipeline consumes the **approved-story** snapshot (`PipelineDependencies.story_repository`, empty-condition names story review), `master_cv_from_any_snapshot` dispatcher (all four consumers incl. interview prep), tailoring `highlight_refs` + evidence-grounded number gate, outreach-body number gate; pipeline/scheduler tests rewritten to seed approved stories (`conftest.seed_approved_story`). Remaining Phase 5b: flag-gated LLM `StorySynthesizer` (fake-tested, one live run credit-gated + `eval_stories`) (built; awaiting review)
- [ ] M20 — V3 Phase 4: render the Master CV from **approved stories** — story-shaped snapshot (`domain/story_snapshot.py`: `STORY_SNAPSHOT_KIND="approved_stories"`, deterministic P/A/R→bullets, render-time resume-ready gate, cross-story `DuplicateMetricError` refusal), `services/story_snapshot.py` (`create_story_snapshot`: evidenced+attested component resolution, per-bullet number gate), `build_story_resume_context` + `render_master_cv_from_stories` into the frozen renderer; full V3 loop test (`tests/test_full_v3_loop.py`) with docx XML assertions (built; awaiting review)
